...
Earning a new badge now triggers a quick confetti burst (via the
canvas-confetti library, loaded from a CDN) alongside the toast
notification. Confetti colors match your current theme's accent
colors, and it's automatically skipped if the visitor's OS has
"reduce motion" turned on.

## Dark mode

There's a theme toggle that cycles through three options:
...
- **Light / Dark** are manual picks that override the system setting
  until you cycle back to System.
