...
## New: reset, charts, export, and a cute theme

- **Reset all data** — a "Danger zone" section at the bottom of the
  dashboard. It deletes every transaction and badge, and clears your
  budget settings, after a confirm dialog **and** typing `RESET` to
  confirm. Meant for wiping test data before you hand the app to real
  users — there's no undo, so don't use it on a real account.
- **Charts** — a donut chart (spending by category) and a bar chart
  (income vs. expenses) for the current month, powered by Chart.js
  (loaded from a CDN, no install needed). They redraw automatically
  when you switch themes so colors stay legible.
- **Export to CSV** — the "Export CSV" button next to the Register
  downloads your full transaction history as a `.csv` file, which
  opens directly in Excel or Google Sheets (File → Import in Sheets).
- **Cute cat theme** — the theme toggle cycles through 🖥 System /
  ☀ Light / ☾ Dark / 🐾 Cute (see the "Dark mode" section below for
  details).

## Dark mode

There's a theme toggle that now cycles through four options:
🖥 System → ☀ Light → ☾ Dark → 🐾 Cute → back to System.

- **System** (the default for new visitors) follows your device's
  light/dark setting automatically, and updates live if you change it
  in your OS while the app is open.
- **Light / Dark / Cute** are manual picks that override the system
  setting until you cycle back to System.

Your choice is saved in the browser's local storage — each device
remembers its own preference, since this isn't synced to your account
in Firestore. The toggle sits top-left on the sign-in page, top-right
in the dashboard header.

The **Cute** theme keeps the exact same layout, spacing, and corner
rounding as Light/Dark — it only swaps in a warm, unisex palette
(terracotta and sage on a soft cream background — no pink), a playful
font pairing (Baloo 2 for headings, Quicksand for body text), and
cat-themed badge icons (🐾 🐟 🧶 👑 🐱). Kept deliberately restrained
so it still feels like the same app, not a different skin bolted on.
